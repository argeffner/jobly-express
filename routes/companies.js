...
router.get("/", async function (req, res, next) {
  ...
  if (quer.minEmployees !== undefined) quer.minEmployees = +quer.minEmployees;
  if (quer.maxEmployees !== undefined) quer.maxEmployees = +quer.maxEmployees;
  try {
    const validator = jsonschema.validate(quer, companySearchSchema);
    if (!validator.valid) {
      const errs = validator.errors.map(e => e.stack);
      throw new BadRequestError(errs);
    }
