...
// change authoriztion to admin-only for all tests
...
describe("GET /users/:username", function () {
  ...
  test("unauth random user", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${randUToken}`);
    expect(resp.statusCode).toEqual(401);
  });
...
describe("POST /users/:username/jobs/:id", function () {
  ...
  test("not found if no such job or job id", async function () {
    const resp = await request(app)
